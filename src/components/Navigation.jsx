...
const Navigation = () => {
    ...
    return (
        <>
            {pathname !== '/signup' && <nav className={`py-[35px] px-[60px] sm:px-[20px] sm:py-0 w-full bg-base ${!hideMenu ? 'sm:fixed sm:left-0 sm:z-50 sm:h-screen' : null} flex items-center justify-between gap-[97px] sm:gap-[0px]`}>
                ...
                <div className='flex flex-row items-center justify-between sm:hidden flex-[2] sm:w-full lg:w-full'>
                    <div className='flex-[2] sm:hidden'>
                        <ul className='flex flex-row gap-[50px] md:gap-5 text-[16px] text-body-text font-ui-regular list-none outline-none transition-all ease-in-out h-full items-center'>
                            <NavLink to={'/'} className={({ isActive }) => isActive ? 'text-primary font-ui-semi transition-all ease-out duration-500' : 'select-none hover:text-primary'}>
                                <li className='w-fit'>Home</li>
                            </NavLink>
                            <NavLink to={'/about'} className={({ isActive }) => isActive ? 'text-primary font-ui-semi transition-all ease-out duration-500' : 'select-none hover:text-primary'}>
                                <li className='w-fit'>About us</li>
                            </NavLink>
                            <NavLink to={'/faq'} className={({ isActive }) => isActive ? 'text-primary font-ui-semi transition-all ease-out duration-500' : 'select-none hover:text-primary'}>
                ...
                <div className={`fixed hidden top-[10vh] left-0 h-screen ${hideMenu ? 'translate-x-[-100%]' : 'translate-x-0'} sm:flex flex-col bg-white px-5 py-5 sm:h-full sm:w-[100vw] transform transition-transform ease-in-out duration-300 z-40 lg:w-full`}>
                    <div className='mb-5'>
                        <ul className='flex flex-col gap-5 text-[16px] font-ui-regular list-none transition-all ease-in-out h-full'>
                            <NavLink to={'/'} className={({ isActive }) => isActive ? 'text-primary font-ui-semi transition-all ease-out duration-500' : 'select-none'} onClick={handleHideModal}>
                                <li className='w-full py-4'>Home</li></NavLink>
                            <NavLink to={'/about'} className={({ isActive }) => isActive ? 'text-primary font-ui-semi transition-all ease-out duration-500' : 'select-none'} onClick={handleHideModal}>
                                <li className='w-full py-4'>About us</li></NavLink>
                            <NavLink to={'/faq'} className={({ isActive }) => isActive ? 'text-primary font-ui-semi transition-all ease-out duration-500' : 'select-none'} onClick={handleHideModal}>
                                <li className='w-full py-4'>Faq</li></NavLink>
                        </ul>
                    </div>
